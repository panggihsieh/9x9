...
老師先輸入後台核發的四碼數字授權密碼，再選擇教室模式與班級代碼開課。未驗證、換 Firebase 身分、重新整理或登入逾八小時，開課按鈕保持停用。Firebase 規則亦檢查有效授權，無法只修改按鈕繞過。班級所有權仍由匿名登入的 `teacherUid` 核對。

班級代碼由瀏覽器自動產生四碼數字，不接受手動輸入。首次進入、練習時間結束，以及結束並釋放班級後都會準備新的代碼；開課交易若遇到正在使用的代碼，會自動換碼重試。學生每次加入都會重新讀取該班的教室模式與倒數狀態。
